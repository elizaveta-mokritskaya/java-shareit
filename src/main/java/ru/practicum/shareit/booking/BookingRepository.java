...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> getBookingByOwnerIdAndEndBefore(Long userId, LocalDateTime date); //PAST booking
    ...
    @Query("select b from Booking as b where b.item.owner.id = :userId and b.start < :date and b.end > :date " +
            "order by b.start desc ")
    List<Booking> getBookingByOwnerIdAndStartIsBeforeAndEndAfter(Long userId, LocalDateTime date); //CURRENT booking

    @Query("select b from Booking as b where b.item.owner.id = :userId and b.status = :status " +
            "order by b.start desc ")
    List<Booking> getBookingByOwner_IdAndStatus(Long userId, Status status); //WAITING ожид.подтв. REJECTED отклонён.
